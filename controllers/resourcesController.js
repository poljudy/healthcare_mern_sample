...
module.exports = {
  getbycatagory: function (req, res) {
    db.Resources.findOne({ catagory: req.params.catagory }, req.body)
      .then((dbresult) => res.json(dbresult))
      .catch((err) => res.status(404).json(err));
  },

  editbycatagory: function (req, res) {
    db.Resources.findOneAndUpdate({ catagory: req.params.catagory }, req.body)
      .then((dbModel) => res.json(dbModel))
      .catch((err) => res.status(422).json(err));
  },

  createbycatagory: function (req, res) {
    db.Resources.create({ catagory: req.params.catagory }, req.body)
      .then((dbModel) => res.json(dbModel))
      .catch((err) => res.status(422).json(err));
  },
